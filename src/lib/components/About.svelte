<script>
</script>
<div class="relative h-screen overflow-hidden">

   <div class="absolute z-1 left-0 top-1/2 -translate-y-1/2">
      <img 
        src="/kim.png" 
        alt="Mr. Kim"
        class="h-40 md:h-64 -translate-x-4/11 hover:translate-x-0 transition-transform"
      />
    </div>

    <div class="z-0 h-screen w-full flex items-center justify-center text-white">
      <p class="text-4xl font-serif backdrop-blur-sm px-6 py-3 rounded-lg">Wine & Lavender </p>
       </div>

    <div class="absolute z-1 right-0 top-1/2 -translate-y-1/2">
      <img 
        src="/olaf.png" 
        alt="Olaf"
        class="h-40 md:h-64 translate-x-2/10 hover:translate-x-0 transition-transform"
      />
    </div>
</div>

